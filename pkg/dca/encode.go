...
type EncodeOptions struct {
	...
	Bitrate          int              // audio encoding bitrate in b/s can be 8000 - 128000
...
func (e *EncodeSession) run() {
	...
	args := []string{
		...
		"-map", "0:a",
		//"-af", "atempo=1.06,asetrate=48000*1.25",
		"-acodec", "libopus",
		"-f", "ogg",
		"-vbr", vbrStr,
	...
	*/
	...
	/*ffmpeg := exec.Command("ffmpeg", "-i", inFile,
		"-acodec", "libopus",
		"-b:a", strconv.Itoa(e.options.Bitrate),
		"-ar", strconv.Itoa(e.options.FrameRate),
		//"-af", fmt.Sprint("atempo=1.06,asetrate=", e.options.FrameRate, "*1.25"),
		"-ac", strconv.Itoa(e.options.Channels),
		"-compression_level", strconv.Itoa(e.options.CompressionLevel),
		"-vbr", "on",
		"-frame_duration", strconv.Itoa(e.options.FrameDuration),
